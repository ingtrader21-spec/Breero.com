...
POLICY_RULES: Final[tuple[EndpointPolicyRule, ...]] = (
    ...
    _rule(
        'access-context-self-read',
        r'/api/v1/auth/access/me',
        methods=_methods('GET'),
        resource_owner='identity',
        audience='authenticated-user',
        authentication='bearer',
        permission='authenticated-session',
        tenant_scope='requested-brand-context',
        record_policy='current-subject-effective-access-context',
        capability_gate='always',
    ),
    _rule(
        'portal-capabilities-read',
        r'/api/v1/portal/capabilities',
        methods=_methods('GET'),
        resource_owner='portal',
        audience='authenticated-portal-user',
        authentication='bearer',
        permission='authenticated-session',
        tenant_scope='effective-session-context',
        record_policy='current-runtime-capability-view',
        capability_gate='always',
    ),
    _rule(
        'portal-provider-overview-read',
        r'/api/v1/portal/provider/overview',
        methods=_methods('GET'),
        resource_owner='provider-portal',
        audience='provider',
        authentication='bearer',
        permission='provider.profile.read',
        tenant_scope='provider-membership',
        record_policy='current-provider-organization-only',
        capability_gate='always',
    ),
    _rule(
        'portal-provider-jobs-read',
        r'/api/v1/portal/provider/jobs',
        methods=_methods('GET'),
        resource_owner='provider-portal',
        audience='provider',
        authentication='bearer',
        permission='provider.jobs.read',
        tenant_scope='provider-membership',
        record_policy='current-provider-jobs-only',
        capability_gate='always',
    ),
    _rule(
        'portal-provider-workers-read',
        r'/api/v1/portal/provider/workers',
        methods=_methods('GET'),
        resource_owner='provider-portal',
        audience='provider',
        authentication='bearer',
        permission='provider.workers.read',
        tenant_scope='provider-membership',
        record_policy='current-provider-workers-only',
        capability_gate='always',
    ),
    _rule(
        'portal-provider-credentials-read',
        r'/api/v1/portal/provider/credentials',
        methods=_methods('GET'),
        resource_owner='provider-portal',
        audience='provider',
        authentication='bearer',
        ...
        tenant_scope='provider-membership',
        record_policy='current-provider-credential-metadata-only',
        capability_gate='always',
    ),
    _rule(
        'portal-provider-earnings-read',
        r'/api/v1/portal/provider/earnings',
        methods=_methods('GET'),
        resource_owner='provider-finance-read-model',
        audience='provider',
        authentication='bearer',
        permission='provider.earnings.read',
        tenant_scope='provider-membership',
        record_policy='current-provider-earnings-only',
        capability_gate='read-only-while-payments-disabled',
    ),
    _rule(
        'portal-provider-payout-batches-read',
        r'/api/v1/portal/provider/payout-batches',
        methods=_methods('GET'),
        resource_owner='provider-finance-read-model',
        audience='provider',
        authentication='bearer',
        permission='provider.payouts.read',
        tenant_scope='provider-membership',
        record_policy='current-provider-payout-batches-only',
        capability_gate='read-only-while-payouts-disabled',
    ),
    _rule(
        'portal-operations-overview-read',
        r'/api/v1/portal/operations/overview',
        methods=_methods('GET'),
        resource_owner='operations-portal',
        audience='operations',
        authentication='bearer',
        permission='ops.dispatch.read',
        tenant_scope='authorized-operations-scope',
        record_policy='authorized-operations-summary',
        capability_gate='always',
    ),
    _rule(
        'portal-admin-overview-read',
        r'/api/v1/portal/admin/overview',
        methods=_methods('GET'),
        resource_owner='admin-portal',
        audience='admin',
        authentication='bearer',
        permission='admin.capabilities.read & admin.audit.read & admin.access.manage & admin.integrations.read & ops.dispatch.read & ops.bookings.read & ops.providers.read & ops.customers.read & finance.ledger.read & finance.payouts.read',
        tenant_scope='authorized-admin-scope',
        record_policy='complete-cross-domain-admin-read-only-view',
        capability_gate='all-required-permissions; no write capability implied',
    ),
    _rule(
        'portal-admin-audit-read',
        r'/api/v1/portal/admin/audit',
        methods=_methods('GET'),
        ...
        tenant_scope='authorized-admin-scope',
        record_policy='authorized-audit-events',
        capability_gate='always',
    ),
    _rule(
