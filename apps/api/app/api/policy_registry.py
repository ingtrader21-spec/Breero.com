...
POLICY_RULES: Final[tuple[EndpointPolicyRule, ...]] = (
    ...
    _rule(
        "tenant-email-config-read",
        r"/api/v1/email/(?:domains|senders|credentials)",
        methods=_methods("GET"),
        resource_owner="tenant-email",
        audience="authorized-email-operator",
        authentication="bearer",
        permission="route-defined email.*.read or email.*.manage",
        tenant_scope="brand-or-authorized-vendor",
        record_policy="effective-permission plus tenant-record-scope",
        capability_gate="always; read/configuration only",
        emitted_effect="none",
        rate_limit_class="authenticated-read",
        pii_classification="confidential-email-config",
    ),
    _rule(
        "tenant-email-config-manage",
        r"/api/v1/email/(?:domains|senders|credentials)",
        methods=_methods("POST"),
        resource_owner="tenant-email",
        audience="authorized-email-operator",
        authentication="bearer",
        permission="route-defined email.*.manage",
        tenant_scope="brand-or-authorized-vendor",
        record_policy="effective-permission plus tenant-record-scope",
        capability_gate="configuration only; does not activate live delivery",
        write_idempotency_key_policy="resource-uniqueness-or-route-defined",
        write_request_hash_policy="required-target",
        write_if_match_version_policy="not-yet-enforced",
        emitted_effect="tenant-email-configuration-changed",
        rate_limit_class="privileged-command",
        pii_classification="restricted-email-credentials-metadata",
    ),
    _rule(
        "tenant-email-domain-verification",
        r"/api/v1/email/domains/{domain_id}/verification",
        methods=_methods("POST"),
        resource_owner="tenant-email",
        audience="trusted-email-verifier",
        authentication="bearer",
        permission="email.domain.verify",
        tenant_scope="brand-or-authorized-vendor",
        record_policy="effective-permission plus tenant-record-scope",
        capability_gate="verification only; live delivery remains separately gated",
        write_idempotency_key_policy="state-transition-idempotent-target",
        write_request_hash_policy="required-target",
        write_if_match_version_policy="not-yet-enforced",
        emitted_effect="email-domain-verification-state",
        rate_limit_class="privileged-command",
        pii_classification="internal",
    ),
    _rule(
        "tenant-email-message-compose",
        r"/api/v1/email/messages",
        methods=_methods("POST"),
        resource_owner="tenant-email",
        audience="authorized-email-composer",
        authentication="bearer",
        permission="email.message.compose",
        tenant_scope="brand-or-authorized-vendor",
        record_policy="effective-permission plus tenant-record-scope",
        capability_gate=(
            "compose is durable; provider delivery requires "
            "settings.email_enabled && settings.transactional_email_mode != disabled"
        ),
        write_idempotency_key_policy="required",
        write_request_hash_policy="idempotency-payload-scope",
        write_if_match_version_policy="not-applicable-on-create",
        emitted_effect="email.message.queued durable outbox event",
        rate_limit_class="customer-command",
        pii_classification="customer-sensitive",
    ),
    _rule(
        "tenant-email-message-read",
        r"/api/v1/email/messages/{message_id}",
        methods=_methods("GET"),
        resource_owner="tenant-email",
        audience="authorized-email-operator",
        authentication="bearer",
        permission="email.message.read or email.message.compose",
        tenant_scope="brand-or-authorized-vendor",
        record_policy="effective-permission plus tenant-record-scope",
        capability_gate="always",
        emitted_effect="none",
    ...
    _rule(
        "tenant-email-outbox-read",
        r"/api/v1/email/outbox",
        methods=_methods("GET"),
        resource_owner="tenant-email-delivery",
        audience="authorized-email-operator",
        authentication="bearer",
        permission="email.outbox.read",
        tenant_scope="brand-or-authorized-vendor",
        record_policy="tenant-filtered email-message outbox rows",
        capability_gate="always",
        emitted_effect="none",
        rate_limit_class="internal-read",
        pii_classification="operational",
    ...
    _rule(
        "tenant-email-outbox-retry",
        r"/api/v1/email/outbox/{event_id}/retry",
        methods=_methods("POST"),
        resource_owner="tenant-email-delivery",
        audience="authorized-email-operator",
        authentication="bearer",
        permission="email.outbox.retry",
        tenant_scope="brand-or-authorized-vendor",
        record_policy="tenant-owned email-message outbox event",
        capability_gate=(
            "retry only requeues; provider delivery remains gated by "
            "settings.email_enabled && settings.transactional_email_mode != disabled"
        ),
        write_idempotency_key_policy="event-id retry state",
        write_request_hash_policy="not-applicable",
        write_if_match_version_policy="outbox-lease-and-event-state",
        emitted_effect="outbox-event-requeued",
        rate_limit_class="privileged-command",
        pii_classification="operational",
    ),
    _rule(
