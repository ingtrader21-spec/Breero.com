...
export function AuthForm({ mode }: { mode: Mode }) {
    ...
    try {
      if (mode === "login") {
        ...
        const session = await customerApi.auth.register({
          full_name: `${data.get("first_name")} ${data.get("last_name")}`.trim(),
          email: String(data.get("email")),
          password: String(data.get("password")),
        });
        customerSession.save(session);
        ...
        await customerApi.auth.resetPassword({
  ...
  if (state === "success") {
    const accountReady = mode === "login" || mode === "register";
    return (
      <div className="approval-success" role="status">
        ...
        <p>
          {mode === "forgot"
            ...
            : mode === "verify"
              ? "Your email is confirmed. You can now use your BREERO account."
              : mode === "reset"
  ...
  return (
    <form action={submit} aria-busy={state === "loading"}>
      {mode === "register" && (
      ...
      {(["login", "register", "reset"] as Mode[]).includes(mode) && (
        <FormField
          label={mode === "reset" ? "New password" : "Password"}
          htmlFor="password"
          hint={mode !== "login" ? "Use at least 10 characters." : undefined}
          required
        >
          <Input
            id="password"
      ...
      {mode === "login" && (
        <div className="auth-options">
          <Checkbox label="Keep me signed in for this browser session" />
      ...
      {state === "error" && (
        <p className="auth-message auth-error" role="alert">
          {mode === "login"
            ? "Those details don’t match. Check them and try again."
      ...
      <Button
        ...
        disabled={(mode === "verify" || mode === "reset") && !query.get("token")}
      >
        {mode === "login"
          ? "Sign in"
          : mode === "register"
